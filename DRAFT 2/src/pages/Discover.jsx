...
const C = { card:'#161616', border:'#2A2A2A', accent:'#C8FF00', muted:'#888', sub:'#555', surface:'#0D0D0D' }
...
const ACCENT_BY_POS = { PG:C.accent, SG:'#FF2D78', SF:'#00C2FF', PF:'#00E676', C:'#9B5CFF', Scout:'#FF6B00', Coach:'#FF6B00' }
...
export default function Discover({ session }) {
  ...
  useEffect(()=>{
    if (!q) {
      supabase.from('profiles').select('*').order('created_at', { ascending:false }).limit(50).then(({data})=>setPlayers(data||[]))
      return
    }
  ...
  const filtered = players.filter(p => pos==='All' || p.position===pos || (pos==='Scout/Coach' && (p.role==='scout'||p.role==='coach')))

  return (
    <div style={{ height:'100%', overflowY:'auto', padding:'20px 16px 10px' }}>
      ...
      <div style={{ position:'relative', marginBottom:12 }}>
        <div style={{ position:'absolute', left:14, top:'50%', transform:'translateY(-50%)', opacity:.4 }}>
        ...
        <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search players, schools…"
          style={{ width:'100%', background:C.card, border:`1.5px solid ${C.border}`, borderRadius:12, padding:'12px 14px 12px 42px', color:'#fff', fontSize:15, outline:'none' }}
      ...
      <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:18 }}>
        {['All','PG','SG','SF','PF','C','Scout/Coach'].map(f=>(
          <button key={f} onClick={()=>setPos(f)}
            style={{ padding:'6px 14px', borderRadius:20, border:`1.5px solid ${pos===f?'transparent':C.border}`, background:pos===f?C.accent:'transparent', color:pos===f?'#000':C.muted, fontSize:12, fontWeight:700, transition:'all .18s' }}>
            {f}
          </button>
        ))}
      </div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        {filtered.map(p=>{
          const accent = ACCENT_BY_POS[p.position] || ACCENT_BY_POS[p.role] || C.accent
          return (
            <div key={p.id} onClick={()=>navigate(`/profile/${p.id}`)}
              style={{ background:C.card, borderRadius:14, padding:'14px', display:'flex', gap:13, alignItems:'center', cursor:'pointer', border:`1px solid ${C.border}`, transition:'border-color .18s' }}
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ fontSize:13, color:C.muted, marginBottom:p.ppg?8:0 }}>
                  {[p.position, p.school, p.year].filter(Boolean).join(' · ')}
                </div>
              </div>
              {p.position && (
                <div style={{ background:`${accent}15`, border:`1.5px solid ${accent}44`, borderRadius:10, padding:'6px 11px', textAlign:'center', flexShrink:0 }}>
                  <div style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:16, fontWeight:700, color:accent }}>{p.position}</div>
                </div>
              )}
            </div>
          )
