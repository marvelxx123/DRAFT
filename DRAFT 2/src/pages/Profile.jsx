...
import { getProfile, getUserVideos, signOut, updateProfile } from '../lib/supabase.js'
...
function Av({ profile, size=80, accent=C.accent, ring=false }) {
  const name     = profile?.full_name || profile?.username || '?'
  const initials = name.split(' ').map(w=>w[0]).join('').slice(0,2).toUpperCase()
  const outer    = ring ? size+6 : size
  return (
    <div style={{ width:outer, height:outer, borderRadius:'50%', flexShrink:0, background:ring?accent:'transparent', padding:ring?3:0, display:'flex', alignItems:'center', justifyContent:'center' }}>
      {profile?.avatar_url
...
export default function Profile({ session }) {
  ...
  if (!targetId) return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:16, padding:24 }}>
      ...
      <p style={{ color:C.muted, textAlign:'center', fontSize:15 }}>Create your player profile and get discovered by scouts</p>
      <Link to="/register" style={{ background:C.accent, color:'#000', borderRadius:12, padding:'13px 28px', fontWeight:800, fontSize:15, textDecoration:'none' }}>Create Free Profile</Link>
  ...
  const inp = { background:C.card, border:`1.5px solid ${C.border}`, borderRadius:10, padding:'11px 14px', color:'#fff', fontSize:14, outline:'none', width:'100%' }
    ...
    <div style={{ height:'100%', overflowY:'auto' }}>
      <div style={{ padding:'20px 18px 0', maxWidth:560, margin:'0 auto' }}>
        ...
        <div style={{ display:'flex', gap:20, alignItems:'flex-start', marginBottom:16 }}>
          <Av profile={profile} size={80} accent={accent} ring/>
          <div style={{ flex:1 }}>
              ...
              {isOwn
                ? <>
                    ...
                    <button onClick={handleSignOut}
                      style={{ padding:'9px 14px', borderRadius:10, fontWeight:700, fontSize:14, background:'transparent', color:C.muted, border:`1.5px solid ${C.border}` }}>
                      Sign out
                    </button>
                  </>
        ...
        {!editing ? (
          <div style={{ marginBottom:16 }}>
            ...
            {profile?.position && <div style={{ fontSize:14, fontWeight:600, color:accent, marginBottom:6 }}>{[profile.position, profile.school, profile.year].filter(Boolean).join(' · ')}</div>}
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10, marginBottom:16, background:C.card, borderRadius:14, padding:16, border:`1px solid ${C.border}` }}>
            <input placeholder="Full name" value={editForm.full_name} onChange={e=>setEF(f=>({...f,full_name:e.target.value}))} style={inp}/>
            <input placeholder="School or team" value={editForm.school} onChange={e=>setEF(f=>({...f,school:e.target.value}))} style={inp}/>
            <input placeholder="Year (e.g. Junior)" value={editForm.year} onChange={e=>setEF(f=>({...f,year:e.target.value}))} style={inp}/>
            <input placeholder="Height (e.g. 6'2)" value={editForm.height} onChange={e=>setEF(f=>({...f,height:e.target.value}))} style={inp}/>
            <textarea placeholder="Bio" value={editForm.bio} onChange={e=>setEF(f=>({...f,bio:e.target.value}))} rows={3} style={{...inp,resize:'none'}}/>
        ...
        <div style={{ maxWidth:560, margin:'0 auto' }}>
          {videos.length===0 && (
            <div style={{ padding:'48px 20px', textAlign:'center', color:C.muted }}>
              ...
              {isOwn && <Link to="/upload" style={{ display:'inline-block', marginTop:16, background:C.accent, color:'#000', borderRadius:10, padding:'11px 24px', fontWeight:800, fontSize:14, textDecoration:'none' }}>Upload Your First Video</Link>}
            </div>
          )}
          {videos.map(v=>(
            <div key={v.id} style={{ padding:'14px 18px', borderBottom:`1px solid ${C.border}`, display:'flex', gap:13, alignItems:'center' }}>
              <div style={{ width:62, height:62, borderRadius:12, background:C.card, border:`1px solid ${C.border}`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, overflow:'hidden', position:'relative' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:4, flexShrink:0 }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill={C.pink}><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
