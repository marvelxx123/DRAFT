...
export default function BottomNav({ session }) {
  ...
  return (
    <nav style={{ background:`${C.surface}f8`, backdropFilter:'blur(20px)', WebkitBackdropFilter:'blur(20px)', borderTop:`1px solid ${C.border}`, display:'flex', justifyContent:'space-around', padding:'10px 0 16px', flexShrink:0, paddingBottom:'max(16px, env(safe-area-inset-bottom))' }}>
      {TABS.map(t => {
        const on = pathname===t.path || (t.path!=='/' && pathname.startsWith(t.path))
        return (
          <button key={t.path} onClick={() => navigate(t.path)}
            style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:4, background:'none', border:'none', padding:'3px 20px', WebkitTapHighlightColor:'transparent', position:'relative' }}>
            <Ic n={t.icon} size={23} color={on?C.accent:C.muted} sw={on?2:1.7}/>
            <span style={{ fontSize:10, fontWeight:700, letterSpacing:.5, textTransform:'uppercase', color:on?C.accent:C.muted, transition:'color .18s' }}>
              {t.label}
            </span>
