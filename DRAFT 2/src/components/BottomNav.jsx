...
const C = { border:'#2A2A2A', accent:'#C8FF00', muted:'#555', surface:'#0D0D0D' }
...
export default function BottomNav({ session }) {
  ...
  return (
    <nav style={{ background:`${C.surface}f9`, backdropFilter:'blur(20px)', borderTop:`1px solid ${C.border}`, display:'flex', justifyContent:'space-around', padding:'10px 0 14px', flexShrink:0 }}>
      {TABS.map(t => {
        const on = pathname === t.path || (t.path !== '/' && pathname.startsWith(t.path))
        return (
          <button key={t.path} onClick={() => navigate(t.path)}
            style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:4, background:'none', border:'none', padding:'3px 18px', position:'relative' }}>
