...
const ACC = { PG:C.accent, SG:C.pink, SF:C.blue, PF:C.green, C:C.purple }
...
/* ── Avatar ── */
function Av({ profile, size=44, accent=C.accent }) {
...
/* ── Score Badge ── */
function Score({ val, accent=C.accent }) {
  return (
    <div style={{ background:`${accent}15`, border:`1.5px solid ${accent}44`, borderRadius:10, padding:'6px 11px', textAlign:'center', flexShrink:0, minWidth:52 }}>
      ...
      <div style={{ fontSize:9, color:C.muted, fontWeight:700, letterSpacing:1.2, marginTop:2 }}>DRAFT</div>
    </div>
  )
}
...
function NotesModal({ player, notes, onSave, onClose }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.85)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ background:C.card, borderRadius:16, border:`1px solid ${C.border}`, width:'100%', maxWidth:440, overflow:'hidden' }}>
        <div style={{ padding:'18px 20px', borderBottom:`1px solid ${C.border}`, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div>
            ...
            <div style={{ fontSize:12, color:C.sub, marginTop:2 }}>{player?.full_name}</div>
          </div>
          <button onClick={onClose} style={{ background:C.card2, border:'none', borderRadius:'50%', width:30, height:30, display:'flex', alignItems:'center', justifyContent:'center' }}>
            <Ic n="close" size={14} color={C.sub}/>
          </button>
        </div>
        <div style={{ padding:20 }}>
          <textarea value={text} onChange={e=>setText(e.target.value)} rows={6}
            ...
            style={{ width:'100%', background:C.surface, border:`1.5px solid ${C.border}`, borderRadius:10, padding:'12px 14px', color:C.text, fontSize:14, outline:'none', resize:'none', lineHeight:1.6, fontFamily:'Inter,sans-serif' }}
            onFocus={e=>e.target.style.borderColor=C.accent} onBlur={e=>e.target.style.borderColor=C.border}/>
          <div style={{ display:'flex', gap:10, marginTop:12 }}>
            <button onClick={onClose} style={{ flex:1, padding:'11px 0', borderRadius:10, background:'transparent', border:`1.5px solid ${C.border}`, color:C.sub, fontWeight:700, fontSize:14 }}>Cancel</button>
            <button onClick={()=>{ onSave(text); onClose() }} style={{ flex:2, padding:'11px 0', borderRadius:10, background:C.accent, border:'none', color:'#000', fontWeight:800, fontSize:14 }}>Save Notes</button>
          </div>
        </div>
      </div>
    </div>
...
function NewBoardModal({ onSave, onClose }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.85)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ background:C.card, borderRadius:16, border:`1px solid ${C.border}`, width:'100%', maxWidth:380, padding:24 }}>
        ...
        <input value={name} onChange={e=>setName(e.target.value)} placeholder='e.g. "PG Targets 2026" or "Final 5"'
          style={{ width:'100%', background:C.surface, border:`1.5px solid ${C.border}`, borderRadius:10, padding:'12px 14px', color:C.text, fontSize:15, outline:'none', marginBottom:14, fontFamily:'Inter,sans-serif' }}
          onFocus={e=>e.target.style.borderColor=C.accent} onBlur={e=>e.target.style.borderColor=C.border}
          autoFocus onKeyDown={e=>e.key==='Enter'&&name.trim()&&onSave(name.trim())}/>
        <div style={{ display:'flex', gap:10 }}>
          <button onClick={onClose} style={{ flex:1, padding:'11px 0', borderRadius:10, background:'transparent', border:`1.5px solid ${C.border}`, color:C.sub, fontWeight:700, fontSize:14 }}>Cancel</button>
          <button onClick={()=>name.trim()&&onSave(name.trim())} style={{ flex:2, padding:'11px 0', borderRadius:10, background:C.accent, border:'none', color:'#000', fontWeight:800, fontSize:14 }}>Create Board</button>
        </div>
      </div>
    </div>
  )
...
function ScoutPlayerCard({ player: p, saved, onSave, onNote, onCompare, inCompare, note, onView }) {
  const accent   = ACC[p.position] || C.accent
  ...
  return (
    <div style={{ background:C.card, borderRadius:14, border:`1.5px solid ${inCompare?accent:C.border}`, padding:'16px', transition:'border-color .18s', position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ display:'flex', gap:13, alignItems:'flex-start', marginBottom:12 }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ fontSize:12, color:C.sub, marginBottom:6 }}>{[p.position, p.school, p.year].filter(Boolean).join(' · ')}</div>
          {p.height && <span style={{ fontSize:11, color:C.sub, background:C.card2, padding:'2px 8px', borderRadius:5, border:`1px solid ${C.border}` }}>📏 {p.height}</span>}
        </div>
      ...
      {(p.ppg||p.apg||p.rpg) && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:7, marginBottom:12 }}>
          {[['PPG',p.ppg],['APG',p.apg],['RPG',p.rpg]].map(([k,v])=>v?(
            <div key={k} style={{ background:C.surface, borderRadius:8, padding:'8px 4px', textAlign:'center', border:`1px solid ${C.border}` }}>
              <div style={{ fontFamily:"'Space Mono',monospace", fontSize:18, color:C.text, fontWeight:700, lineHeight:1 }}>{v}</div>
              <div style={{ fontSize:9, color:C.muted, fontWeight:700, letterSpacing:1, marginTop:2 }}>{k}</div>
            </div>
          ):null)}
        </div>
      )}
      ...
      {/* Note preview */}
      {note && (
        <div style={{ background:C.surface, borderRadius:8, padding:'8px 10px', marginBottom:10, border:`1px solid ${C.border}` }}>
          <div style={{ fontSize:11, color:C.muted, fontWeight:700, letterSpacing:.8, textTransform:'uppercase', marginBottom:4 }}>Your Notes</div>
          <p style={{ fontSize:12, color:C.text2, lineHeight:1.5, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{note}</p>
        </div>
      )}

      {/* Action buttons */}
      <div style={{ display:'flex', gap:7 }}>
        <button onClick={()=>onView(p.id)} style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', gap:5, padding:'9px 0', borderRadius:9, background:C.accent, border:'none', color:'#000', fontWeight:800, fontSize:12 }}>
          <Ic n="eye" size={13} color="#000"/>View
        </button>
        <button onClick={()=>onSave(p)} title={saved?'Remove from board':'Save to board'}
          style={{ width:36, height:36, borderRadius:9, display:'flex', alignItems:'center', justifyContent:'center', background:saved?`${C.accent}20`:C.card2, border:`1.5px solid ${saved?C.accent:C.border}`, cursor:'pointer' }}>
          <Ic n={saved?'star-f':'star'} size={14} color={saved?C.accent:C.sub}/>
        </button>
        <button onClick={()=>onNote(p)} title="Add notes"
          style={{ width:36, height:36, borderRadius:9, display:'flex', alignItems:'center', justifyContent:'center', background:note?`${C.blue}20`:C.card2, border:`1.5px solid ${note?C.blue:C.border}`, cursor:'pointer' }}>
          <Ic n="note" size={14} color={note?C.blue:C.sub}/>
        </button>
        <button onClick={()=>onCompare(p)} title="Compare"
          style={{ width:36, height:36, borderRadius:9, display:'flex', alignItems:'center', justifyContent:'center', background:inCompare?`${C.purple}20`:C.card2, border:`1.5px solid ${inCompare?C.purple:C.border}`, cursor:'pointer' }}>
          <Ic n="compare" size={14} color={inCompare?C.purple:C.sub}/>
        </button>
      </div>

      {/* Video count badge */}
      {videoCount > 0 && (
        <div style={{ position:'absolute', top:14, right:14, background:C.card2, border:`1px solid ${C.border}`, borderRadius:6, padding:'2px 7px', display:'flex', alignItems:'center', gap:4 }}>
          <Ic n="play" size={9} color={C.muted}/>
          <span style={{ fontSize:10, color:C.muted, fontWeight:700 }}>{videoCount}</span>
        </div>
      )}
    </div>
  )
...
function ComparePanel({ players, onRemove }) {
  ...
  return (
    <div style={{ background:C.surface, borderRadius:14, border:`1px solid ${C.border}`, padding:'18px 16px', marginBottom:18 }}>
      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:14 }}>
        <Ic n="compare" size={16} color={C.purple}/>
        <span style={{ fontFamily:"'Space Grotesk',sans-serif", fontWeight:700, fontSize:15, color:C.text }}>Comparing {players.length} Players</span>
      </div>
        ...
        {players.map(p=>{
          ...
          return (
            <div key={p.id} style={{ textAlign:'center' }}>
              ...
              <div style={{ fontSize:12, fontWeight:700, marginTop:6, color:C.text }}>{p.full_name?.split(' ')[0]}</div>
              <div style={{ fontSize:10, color:C.sub }}>{p.position}</div>
              <div style={{ fontFamily:"'Space Mono',monospace", fontSize:16, color:accent, fontWeight:700, marginTop:4 }}>{p.draft_score||'—'}</div>
              <div style={{ fontSize:9, color:C.muted }}>DRAFT SCORE</div>
            </div>
          )
        })}
      </div>
      ...
      {cats.map(cat=>{
        ...
        return (
          <div key={cat} style={{ marginBottom:10 }}>
            <div style={{ fontSize:10, fontWeight:700, color:C.muted, letterSpacing:1.2, textTransform:'uppercase', marginBottom:6 }}>{cat}</div>
                ...
                return (
                  <div key={p.id}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:4 }}>
                      <span style={{ fontFamily:"'Space Mono',monospace", fontSize:14, fontWeight:700, color:isTop?accent:C.text }}>{val||'—'}</span>
                      {isTop && max>0 && <span style={{ fontSize:9, fontWeight:800, color:accent }}>BEST</span>}
                    </div>
                    <div style={{ height:4, background:C.border, borderRadius:2 }}>
                      <div style={{ height:4, borderRadius:2, background:isTop?accent:C.sub, width:max>0?`${(val/max)*100}%`:'0%', transition:'width .5s' }}/>
                    </div>
                  </div>
                )
              })}
...
export default function ScoutDashboard({ session }) {
  ...
  const inp = { width:'100%', background:C.card, border:`1.5px solid ${C.border}`, borderRadius:12, padding:'12px 14px 12px 42px', color:C.text, fontSize:15, outline:'none', WebkitAppearance:'none' }

  return (
    <div style={{ height:'100%', overflowY:'auto', WebkitOverflowScrolling:'touch', background:C.bg }}>
      ...
      <div style={{ padding:'20px 18px 0' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:4 }}>
          <button onClick={()=>navigate(-1)} style={{ background:'none', border:'none', display:'flex', alignItems:'center', justifyContent:'center' }}>
            <Ic n="back" size={20} color={C.sub}/>
          </button>
          <div style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:26, fontWeight:700, letterSpacing:-.5 }}>Scout Dashboard</div>
          {session?.user && <div style={{ marginLeft:'auto', background:`${C.accent}15`, border:`1px solid ${C.accent}33`, borderRadius:8, padding:'4px 10px' }}>
            <span style={{ fontSize:11, fontWeight:700, color:C.accent }}>SCOUT</span>
          </div>}
        </div>
        <p style={{ fontSize:13, color:C.sub, marginBottom:18, paddingLeft:30 }}>Find your next player. Save time. Make better decisions.</p>

        {/* Stats bar */}
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginBottom:18 }}>
          {[[saved.length,'Saved Players'],[boards.length,'My Boards'],[Object.keys(notes).length,'Notes']].map(([v,k])=>(
            <div key={k} style={{ background:C.card, borderRadius:12, padding:'12px 10px', textAlign:'center', border:`1px solid ${C.border}` }}>
              <div style={{ fontFamily:"'Space Mono',monospace", fontSize:22, fontWeight:700, color:C.accent }}>{v}</div>
              <div style={{ fontSize:10, color:C.sub, fontWeight:600, marginTop:3 }}>{k}</div>
            </div>
          ))}
        </div>

        {/* View tabs */}
        <div style={{ display:'flex', gap:2, background:C.card, borderRadius:12, padding:4, marginBottom:18, border:`1px solid ${C.border}` }}>
          {[['search','🔍 Search'],['boards','📋 My Boards'],['alerts','🔔 Alerts']].map(([id,lbl])=>(
            <button key={id} onClick={()=>setView(id)}
              style={{ flex:1, padding:'9px 0', borderRadius:9, border:'none', background:view===id?C.card2:'transparent', color:view===id?C.text:C.muted, fontWeight:700, fontSize:12, transition:'all .18s' }}>
              {lbl}
            </button>
          ))}
        </div>
      ...
      {view==='search' && (
        <div style={{ padding:'0 18px 20px' }}>
          ...
          <div style={{ display:'flex', gap:6, overflowX:'auto', paddingBottom:8, marginBottom:8, WebkitOverflowScrolling:'touch' }}>
            {POSITIONS.map(p=>(
              <button key={p} onClick={()=>setPos(p)}
                style={{ padding:'6px 13px', borderRadius:18, border:`1.5px solid ${pos===p?'transparent':C.border}`, background:pos===p?C.accent:'transparent', color:pos===p?'#000':C.sub, fontSize:12, fontWeight:700, flexShrink:0, transition:'all .18s' }}>
                {p}
              </button>
            ))}
          </div>
          ...
          <div style={{ display:'flex', gap:6, overflowX:'auto', paddingBottom:8, marginBottom:14, WebkitOverflowScrolling:'touch' }}>
            {LEVELS.map(l=>(
              <button key={l} onClick={()=>setLevel(l)}
                style={{ padding:'6px 13px', borderRadius:18, border:`1.5px solid ${level===l?'transparent':C.border}`, background:level===l?C.blue:'transparent', color:level===l?'#000':C.sub, fontSize:12, fontWeight:700, flexShrink:0, transition:'all .18s' }}>
                {l}
              </button>
            ))}
            {SORT_OPTS.map(s=>(
              <button key={s} onClick={()=>setSort(s)}
                style={{ padding:'6px 13px', borderRadius:18, border:`1.5px solid ${sortBy===s?'transparent':C.border}`, background:sortBy===s?C.purple:'transparent', color:sortBy===s?'#fff':C.sub, fontSize:12, fontWeight:700, flexShrink:0, transition:'all .18s' }}>
                {s}
              </button>
            ))}
          </div>
          ...
          {/* Compare clear */}
          {compare.length > 0 && (
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12, padding:'8px 12px', background:`${C.purple}15`, borderRadius:10, border:`1px solid ${C.purple}33` }}>
              <Ic n="compare" size={14} color={C.purple}/>
              <span style={{ fontSize:12, color:C.purple, fontWeight:700, flex:1 }}>{compare.length} player{compare.length>1?'s':''} selected for comparison</span>
              <button onClick={()=>setCompare([])} style={{ background:'none', border:'none' }}>
                <Ic n="close" size={14} color={C.purple}/>
              </button>
            </div>
          )}
        ...
        <div style={{ padding:'0 18px 20px' }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
            ...
            <button onClick={()=>setBoardModal(true)}
              style={{ display:'flex', alignItems:'center', gap:6, background:C.accent, border:'none', borderRadius:10, padding:'9px 14px', color:'#000', fontWeight:800, fontSize:13 }}>
          ...
          <div style={{ background:C.card, borderRadius:14, border:`1px solid ${C.border}`, marginBottom:14, overflow:'hidden' }}>
            <div style={{ padding:'14px 16px', borderBottom:`1px solid ${C.border}`, display:'flex', alignItems:'center', gap:10 }}>
              <Ic n="star-f" size={16} color={C.accent}/>
              <span style={{ fontFamily:"'Space Grotesk',sans-serif", fontWeight:700, fontSize:15 }}>Saved Players</span>
              <span style={{ marginLeft:'auto', background:`${C.accent}20`, color:C.accent, fontSize:11, fontWeight:800, padding:'2px 8px', borderRadius:6 }}>{saved.length}</span>
            </div>
            {saved.length===0 ? (
              ...
              <div>
                {saved.map(p=>{
                  const accent = ACC[p.position]||C.accent
                  return (
                    <div key={p.id} style={{ padding:'12px 16px', borderBottom:`1px solid ${C.border}`, display:'flex', gap:12, alignItems:'center' }}>
                      <Av profile={p} size={40} accent={accent}/>
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ fontWeight:700, fontSize:14, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{p.full_name||p.username}</div>
                        <div style={{ fontSize:12, color:C.sub }}>{[p.position, p.school].filter(Boolean).join(' · ')}</div>
                      </div>
                      <button onClick={()=>navigate(`/profile/${p.id}`)} style={{ background:C.accent, border:'none', borderRadius:8, padding:'6px 12px', color:'#000', fontWeight:700, fontSize:12 }}>View</button>
                      <button onClick={()=>toggleSave(p)} style={{ background:'none', border:'none' }}>
                        <Ic n="trash" size={15} color={C.muted}/>
                      </button>
                    </div>
                  )
                })}
          ...
          {/* Custom boards */}
          {boards.map(board=>(
            <div key={board.id} style={{ background:C.card, borderRadius:14, border:`1px solid ${C.border}`, marginBottom:14, overflow:'hidden' }}>
              <div style={{ padding:'14px 16px', borderBottom:`1px solid ${C.border}`, display:'flex', alignItems:'center', gap:10 }}>
                <Ic n="board" size={16} color={C.blue}/>
                <span style={{ fontFamily:"'Space Grotesk',sans-serif", fontWeight:700, fontSize:15 }}>{board.name}</span>
                <span style={{ marginLeft:'auto', background:`${C.blue}20`, color:C.blue, fontSize:11, fontWeight:800, padding:'2px 8px', borderRadius:6 }}>{board.players.length}</span>
                <button onClick={()=>deleteBoard(board.id)} style={{ background:'none', border:'none', marginLeft:4 }}>
                  <Ic n="trash" size={14} color={C.muted}/>
                </button>
              </div>
                  ...
                  return (
                    <div key={p.id} style={{ padding:'12px 16px', borderBottom:`1px solid ${C.border}`, display:'flex', gap:12, alignItems:'center' }}>
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        ...
                        <div style={{ fontSize:11, color:C.sub }}>{[p.position, p.school].filter(Boolean).join(' · ')}</div>
                        {notes[p.id] && <div style={{ fontSize:11, color:C.blue, marginTop:2, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>📝 {notes[p.id]}</div>}
                      </div>
                      <button onClick={()=>navigate(`/profile/${p.id}`)} style={{ background:C.blue, border:'none', borderRadius:8, padding:'6px 12px', color:'#fff', fontWeight:700, fontSize:12 }}>View</button>
        ...
        <div style={{ padding:'0 18px 20px' }}>
          ...
          <div style={{ background:C.card, borderRadius:14, border:`1px solid ${C.border}`, overflow:'hidden', marginBottom:14 }}>
            <div style={{ padding:'14px 16px', borderBottom:`1px solid ${C.border}` }}>
              ...
              <div style={{ fontSize:12, color:C.sub }}>Players who joined DRAFT in the last 7 days</div>
            </div>
          ...
          <div style={{ background:C.card, borderRadius:14, border:`1px solid ${C.border}`, overflow:'hidden', marginBottom:14 }}>
            <div style={{ padding:'14px 16px', borderBottom:`1px solid ${C.border}` }}>
              ...
              <div style={{ fontSize:12, color:C.sub }}>Recent uploads from players you saved</div>
            </div>
            {saved.length===0 ? (
              <div style={{ padding:'20px 16px', textAlign:'center', color:C.muted }}>
          ...
          <div style={{ background:`${C.accent}10`, borderRadius:14, border:`1px solid ${C.accent}33`, padding:'16px' }}>
            <div style={{ fontWeight:800, fontSize:14, color:C.accent, marginBottom:6 }}>💡 Coming Soon</div>
...
function NewPlayersAlert({ navigate }) {
  ...
  if (loading) return <div style={{ padding:'16px', textAlign:'center', color:C.muted, fontSize:13 }}>Loading…</div>
  if (players.length===0) return <div style={{ padding:'16px', textAlign:'center', color:C.muted, fontSize:13 }}>No new players this week yet</div>
  ...
  return (
    <div>
      {players.map(p=>{
        const accent = ACC[p.position]||C.accent
        return (
          <div key={p.id} onClick={()=>navigate(`/profile/${p.id}`)} style={{ padding:'12px 16px', borderBottom:`1px solid ${C.border}`, display:'flex', gap:12, alignItems:'center', cursor:'pointer' }}>
            <Av profile={p} size={38} accent={accent}/>
            <div style={{ flex:1 }}>
              <div style={{ fontWeight:700, fontSize:13 }}>{p.full_name||p.username}</div>
              <div style={{ fontSize:11, color:C.sub }}>{[p.position,p.school,p.year].filter(Boolean).join(' · ')}</div>
            </div>
            <span style={{ fontSize:10, color:C.green, fontWeight:700 }}>NEW</span>
          </div>
        )
      })}
    </div>
...
function SavedPlayerVideos({ saved, navigate }) {
  ...
  if (loading) return <div style={{ padding:'16px', textAlign:'center', color:C.muted, fontSize:13 }}>Loading…</div>
  if (videos.length===0) return <div style={{ padding:'16px', textAlign:'center', color:C.muted, fontSize:13 }}>No new videos from saved players</div>

  return (
    <div>
      {videos.map(v=>(
        <div key={v.id} onClick={()=>navigate(`/profile/${v.user_id}`)} style={{ padding:'12px 16px', borderBottom:`1px solid ${C.border}`, display:'flex', gap:12, alignItems:'center', cursor:'pointer' }}>
          <div style={{ width:46, height:46, borderRadius:10, background:C.card2, border:`1px solid ${C.border}`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, overflow:'hidden' }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ fontSize:11, color:C.sub }}>{v.profiles?.full_name} · {v.category}</div>
          </div>
        </div>
      ))}
    </div>
