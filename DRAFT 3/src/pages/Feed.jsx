...
const C = { accent:'#C8FF00', pink:'#FF2D78', blue:'#00C2FF', green:'#00E676', muted:'#888', sub:'#555', surface:'#0D0D0D', card:'#161616', border:'#2A2A2A', border2:'#383838' }
...
const Ic = ({ n, size=24, color='#fff', fill=false, sw=1.8 }) => {
  ...
  switch(n) {
    ...
    case 'play':     return <svg style={s} viewBox="0 0 24 24"><polygon {...f} points="5 3 19 12 5 21 5 3"/></svg>
    case 'pause':    return <svg style={s} viewBox="0 0 24 24"><line {...p} x1="6" y1="4" x2="6" y2="20"/><line {...p} x1="18" y1="4" x2="18" y2="20"/></svg>
    default: return null
  }
}
...
function CommentsPanel({ video, session, onClose }) {
  ...
  useEffect(() => {
    getComments(video.id).then(({ data }) => { setList(data||[]); setLd(false) })
  }, [video.id])
  ...
  return (
    <div style={{ position:'absolute', bottom:0, left:0, right:0, height:'70%', background:C.surface, borderRadius:'20px 20px 0 0', zIndex:100, display:'flex', flexDirection:'column', boxShadow:'0 -2px 40px rgba(0,0,0,.9)', animation:'slideUp .26s cubic-bezier(.32,.72,0,1)' }}>
      <style>{`@keyframes slideUp{from{transform:translateY(100%)}to{transform:translateY(0)}}`}</style>

      {/* Handle */}
      <div style={{ padding:'12px 18px 10px', borderBottom:`1px solid ${C.border}` }}>
        <div style={{ width:36, height:3, background:C.border2, borderRadius:2, margin:'0 auto 12px' }}/>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <span style={{ fontFamily:"'Space Grotesk',sans-serif", fontWeight:700, fontSize:17 }}>{list.length} Comments</span>
          <button onClick={onClose} style={{ background:C.card, border:'none', borderRadius:'50%', width:32, height:32, display:'flex', alignItems:'center', justifyContent:'center' }}>
          ...
          <div key={c.id||i} style={{ display:'flex', gap:11, marginBottom:20 }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ display:'flex', gap:14, marginTop:6 }}>
                <button style={{ background:'none', border:'none', color:C.sub, fontSize:12, fontWeight:600 }}>Reply</button>
      ...
      {/* Input */}
      <div style={{ padding:'10px 14px 16px', borderTop:`1px solid ${C.border}`, display:'flex', gap:10, alignItems:'center' }}>
        {session
          ? <>
              ...
              <button onClick={send} disabled={posting||!text.trim()} style={{ background:'none', border:'none', opacity:(posting||!text.trim())?.3:1, transition:'opacity .18s' }}>
                <Ic n="send" size={22} color={C.accent}/>
              </button>
            </>
          : <p style={{ fontSize:14, color:C.muted, textAlign:'center', width:'100%', padding:'4px 0' }}>
              <span style={{ color:C.accent, fontWeight:700, cursor:'pointer' }} onClick={()=>{}}>Sign in</span> to comment
            </p>
        }
      </div>
    </div>
...
function VideoSlide({ video, session, isActive }) {
  ...
  const togglePause = () => { setPause(p=>!p) }
  ...
  return (
    <div style={{ position:'relative', width:'100%', height:'100%', background:'#000', flexShrink:0, overflow:'hidden' }}>
      {/* Video */}
      <video ref={videoRef} src={video.video_url} loop playsInline
        style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover' }}
        onDoubleClick={doLike} onClick={togglePause}/>

      {/* Pause indicator */}
      {paused && (
        <div style={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', zIndex:50, pointerEvents:'none', background:'rgba(0,0,0,.5)', borderRadius:'50%', width:72, height:72, display:'flex', alignItems:'center', justifyContent:'center' }}>
          <Ic n="play" size={32} color="#fff" fill/>
        </div>
      )}

      {/* Big heart on double tap */}
      {bigHeart && (
        <div style={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', zIndex:60, pointerEvents:'none', animation:'heartPop .65s ease forwards' }}>
          <Ic n="heart" size={100} color={C.pink} fill/>
        </div>
      )}

      {/* Bottom gradient */}
      <div style={{ position:'absolute', inset:0, background:'linear-gradient(to top, rgba(0,0,0,.95) 0%, rgba(0,0,0,.2) 55%, transparent 100%)', pointerEvents:'none', zIndex:10 }}/>
      ...
      <div style={{ position:'absolute', top:0, left:0, right:0, padding:'16px 18px', zIndex:20, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        <div style={{ display:'flex', gap:20 }}>
          {['For You','Following'].map((t,i)=>(
            <button key={t} style={{ background:'none', border:'none', color:i===0?'#fff':'rgba(255,255,255,.38)', fontWeight:700, fontSize:14, paddingBottom:3, borderBottom:i===0?`2px solid ${C.accent}`:'2px solid transparent' }}>{t}</button>
          ))}
        </div>
        <div style={{ width:40 }}/>
      </div>
      ...
      <div style={{ position:'absolute', bottom:16, left:0, width:'78%', padding:'0 16px 18px', zIndex:20 }}>
        <button onClick={() => navigate(`/profile/${profile?.id}`)}
          ...
          <div style={{ flex:1 }}>
            ...
            <div style={{ fontSize:13, color:'rgba(255,255,255,.55)', fontWeight:500 }}>
              {profile?.position && `${profile.position} · `}{profile?.school}
            </div>
          </div>
          {session?.user?.id !== profile?.id && (
            <div style={{ padding:'5px 14px', background:'rgba(255,255,255,.1)', borderRadius:18, border:'1.5px solid rgba(255,255,255,.25)', backdropFilter:'blur(6px)', flexShrink:0 }}>
        ...
        <div style={{ display:'flex', gap:7, flexWrap:'wrap' }}>
          ...
          {profile?.school && <span style={{ fontSize:12, fontWeight:600, padding:'4px 12px', borderRadius:20, background:'rgba(255,255,255,.08)', color:'rgba(255,255,255,.8)', border:'1px solid rgba(255,255,255,.15)' }}>{profile.school}</span>}
        </div>
      </div>
      ...
      <div style={{ position:'absolute', right:14, bottom:22, zIndex:20, display:'flex', flexDirection:'column', gap:26, alignItems:'center' }}>
        ...
        {/* Share */}
        <button onClick={shareVideo} style={{ background:'none', border:'none', display:'flex', flexDirection:'column', alignItems:'center', gap:5 }}>
      ...
      <style>{`@keyframes heartPop{0%{transform:translate(-50%,-50%) scale(0);opacity:1}45%{transform:translate(-50%,-50%) scale(1.8)}70%{transform:translate(-50%,-50%) scale(1.2)}100%{transform:translate(-50%,-50%) scale(1);opacity:0}}`}</style>
    </div>
  )
}
...
export default function Feed({ session }) {
  ...
  if (videos.length===0) return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:14, padding:24, textAlign:'center' }}>
      ...
      <div style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:26, fontWeight:700 }}>No videos yet</div>
      <p style={{ color:C.muted, fontSize:15, maxWidth:280, lineHeight:1.6 }}>Be the first to upload your highlights and get discovered</p>
      {session
        ? <button onClick={() => navigate('/upload')} style={{ background:C.accent, color:'#000', border:'none', borderRadius:12, padding:'14px 30px', fontWeight:800, fontSize:16, marginTop:8 }}>Upload Now 🎬</button>
        : <button onClick={() => navigate('/register')} style={{ background:C.accent, color:'#000', border:'none', borderRadius:12, padding:'14px 30px', fontWeight:800, fontSize:16, marginTop:8 }}>Join DRAFT 🏀</button>
      }
    </div>
  )
  ...
  return (
    <div style={{ position:'relative', width:'100%', height:'100%', overflow:'hidden' }}
      ...
      {/* Sliding container */}
      <div style={{ display:'flex', flexDirection:'column', height:'100%', transform:`translateY(-${idx*100}%)`, transition:'transform .35s cubic-bezier(.32,.72,0,1)', willChange:'transform' }}>
        {videos.map((v,i) => (
