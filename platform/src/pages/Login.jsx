    ...
    <div className="min-h-screen flex items-center justify-center bg-black mesh-bg px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-white/40 mt-3 text-sm">Welcome back</p>
        </div>
        ...
        <div className="glass gradient-border rounded-2xl p-7">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={loading} className="btn-primary w-full justify-center py-3 mt-2">
              {loading ? <Loader2 size={16} className="spin" /> : <>Sign In <ArrowRight size={14} /></>}
            </button>
          </form>
        </div>
        ...
        <p className="text-center text-sm text-white/30 mt-6">
          No account?{' '}
          <Link to="/register" className="text-accent-light hover:underline">Start for free →</Link>
        </p>
      </div>
    </div>
  )
